...
(function () {
  ...
  /* =========================================================
     IMAGE TRAIL — project covers chase the cursor in the hero.
     Vanilla port of the reactbits/Codrops pattern: a rAF loop
     measures pointer travel; every `threshold` px the next cover
     is animated (WAAPI) from the smoothed position to the pointer,
     then fades out. Desktop pointers only.
     ========================================================= */
  function initImageTrail() {
    if (!MOTION_FULL) return;
    const hero = $(".hero");
    const wrap = $(".hero__trail");
    if (!hero || !wrap) return;
    const figures = $$(".hero__trail-img", wrap);
    if (!figures.length) return;

    // Images load only when the effect actually runs (data-src -> src)
    figures.forEach((f) => {
      const img = $("img", f);
    ...
    const spawn = () => {
      ...
      el.animate(
        [
          { transform: `translate(${cache.x - w / 2}px, ${cache.y - h / 2}px) rotate(0deg) scale(.5)`, opacity: 1 },
          { transform: `translate(${mouse.x - w / 2}px, ${mouse.y - h / 2}px) rotate(${rot}deg) scale(1)`, opacity: 1, offset: 0.45 },
          { transform: `translate(${mouse.x - w / 2}px, ${mouse.y - h / 2 + 60}px) rotate(${rot}deg) scale(.85)`, opacity: 0 }
        ],
    ...
    const loop = () => {
      cache.x = lerp(cache.x, mouse.x, 0.25);
      cache.y = lerp(cache.y, mouse.y, 0.25);
